...
class DataManager:
    ...
    def __init__(
        ...
        # Load dataset：从cfg构造数据集。
        dataset = build_dataset(cfg)

        # Build transform
        if custom_tfm_train is None:
            tfm_train = build_transform(cfg, is_train=True)  # 根据cfg.INPUT建立增强器。
        else:
        ...
        # Build train_loader_x：Wrapper数据集（增加图片读取、增强），并根据cfg.DATALOADER建立DataLoader。
