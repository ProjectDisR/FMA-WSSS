...
def build_optimizer(model, optim_cfg, param_groups=None):
    ...
    lr = optim_cfg.LR
    weight_decay = optim_cfg.WEIGHT_DECAY  # 默认0.0005
    momentum = optim_cfg.MOMENTUM  # 默认0.9
    sgd_dampening = optim_cfg.SGD_DAMPNING  # 默认0
    sgd_nesterov = optim_cfg.SGD_NESTEROV  # 默认False
    ...
    if param_groups is None:
        if staged_lr:  # 用于backbone和head使用不同的学习率。
            ...
            if isinstance(model, nn.Module):
                param_groups = model.parameters()  # 模型下辖所有参数。
            else:
                param_groups = model
