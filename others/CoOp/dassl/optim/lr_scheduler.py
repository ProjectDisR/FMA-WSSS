...
class _BaseWarmupScheduler(_LRScheduler):
    ...
    def step(self, epoch=None):
        if self.last_epoch >= self.warmup_epoch:
            self.successor.step(epoch)  # warmup epoch外，step外部（指定epoch模式，其实不推荐）。
            self._last_lr = self.successor.get_last_lr()
        else:
            super().step(epoch)  # warmup epoch内，总是step自身。
...
class ConstantWarmupScheduler(_BaseWarmupScheduler):
    ...
    def get_lr(self):
        if self.last_epoch >= self.warmup_epoch:  # 交接时，任然由WarmUp产生学习率，但给出successor的初始学习率。
...
def build_lr_scheduler(optimizer, optim_cfg):
    ...
    stepsize = optim_cfg.STEPSIZE  # 默认-1
    gamma = optim_cfg.GAMMA  # 默认0.1
