...
class TrainerBase:
    ...
    def train(self, start_epoch, max_epoch):
        ...
        self.before_train()  # resume和建立summary writer。
        for self.epoch in range(self.start_epoch, self.max_epoch):
            self.before_epoch()  # 无事.
            self.run_epoch()
            self.after_epoch()  # 保存模型和测试。
        self.after_train()
    ...
    def model_backward_and_update(self, loss, names=None):
        self.model_zero_grad(names)  # optim清空梯度。
        self.model_backward(loss)  # loss检查是否有inf，然后backward。
        self.model_update(names)  # optim step。
...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        super().__init__()  # 建立模型、优化器、调率器列表。
        self.check_cfg(cfg)  # 检查cfg合法。
        ...
        # Save as attributes some frequently used variables。记录起始、终止epoch、输出路径。
        self.start_epoch = self.epoch = 0
        self.max_epoch = cfg.OPTIM.MAX_EPOCH
        self.output_dir = cfg.OUTPUT_DIR

        self.cfg = cfg
        self.build_data_loader()  # 建立数据集和加载器。
        self.build_model()  # 建立模型、优化器、调率器。
        self.evaluator = build_evaluator(cfg, lab2cname=self.lab2cname)  # 建立分类评价器。
    ...
    def build_data_loader(self):
        ...
        """
        dm = DataManager(self.cfg)  # 从cfg构造数据集，增强器和加载器。
    ...
    def before_train(self):
        ...
        self.init_writer(writer_dir)  # 建立SummaryWriter。
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self):
        self.set_model_mode("train")  # 将注册的模型（注意，只有prompt learner）设为Train状态。
        losses = MetricMeter()
        batch_time = AverageMeter()
        data_time = AverageMeter()
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):  # 获取一个batch数据。
            data_time.update(time.time() - end)
            loss_summary = self.forward_backward(batch)  # 模型前后向。
            batch_time.update(time.time() - end)
            losses.update(loss_summary)
            # * 打印、记录信息。
            meet_freq = (self.batch_idx + 1) % self.cfg.TRAIN.PRINT_FREQ == 0
