...
from alchemy_cat.py_tools import set_rand_seed
...
def resize_labels(labels, size):
    ...
    for label in labels:
        ...
        new_labels.append(np.asarray(label).astype(np.int64))
    new_labels = torch.LongTensor(np.array(new_labels))
    return new_labels


@click.group()  # 主命令，下接子命令，如train、text等。
@click.pass_context
def main(ctx):
    """
...
def train(config_path, cuda):
    ...
    # Setup loss logger
    writer = SummaryWriter(os.path.join(CONFIG.EXP.OUTPUT_DIR, "logs", CONFIG.EXP.ID), purge_step=0)
        ...
        for _ in range(CONFIG.SOLVER.ITER_SIZE):
            ...
            for logit in logits:
                ...
                └─────┘ │                    │ └─────┘ │
                        │ ┌──────────────┐   │         │
                ┌─────┐ │ │              │   │ ┌─────┐l2 ┌─────┐l4
                │x0.75├─┼─┤     model    ├───┼─┤x0.75├─┬─┤ max │
                └─────┘ │ │              │   │ └─────┘ │ └─────┘
                        │  ──────────────┘   │         │
                ┌─────┐ │                    │ ┌─────┐l3
                │ x0.5├─┘                    └─┤ x0.5├──
                └─────┘                        └─────┘
                网络本身就是MSC的，输出max(msc_logits)。训练时三个尺度以及max均收到Loss监督。
                若没有max上loss，则等价于训练时增加了一组MSC增强。且由于同一图的多尺度同时输入网络，网络会更强调尺度不变。
                max上loss令推理-测试监督一致。让网络的多尺度输出能相互配合，在max后给出最优结果。
                ？最好好推理的MSC一致？
                """
                _, _, H, W = logit.shape
...
def test(config_path, model_path, cuda):
    ...
    if model_path is None:
        model_path = os.path.join(CONFIG.EXP.OUTPUT_DIR,
                                  'models',
                                  CONFIG.EXP.ID,
                                  CONFIG.MODEL.NAME.lower(),
                                  CONFIG.DATASET.SPLIT.TRAIN,
    ...
    for image_ids, images, gt_labels in tqdm(
        ...
        gts += list(gt_labels.clone().numpy())
...
@click.option(
    ...
    "--n-jobs",
    type=int,
    default=multiprocessing.cpu_count() // 1.25,
    show_default=True,
