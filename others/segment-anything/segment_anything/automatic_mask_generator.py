...
class SamAutomaticMaskGenerator:
    def __init__(
        ...
        if points_per_side is not None:
            self.point_grids = build_all_layer_point_grids(
                points_per_side,
                crop_n_layers,
                crop_n_points_downscale_factor,  # 各层点数的缩放因子。如为2时，第二层点数为第0层的1/4。
            )
    ...
    def _generate_masks(self, image: np.ndarray) -> MaskData:
        ...
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            crop_data = self._process_crop(image, crop_box, layer_idx, orig_size)  # 每个crop内做grid提示推理+NMS。
            data.cat(crop_data)  # 拼接每个crop的结果。

        # Remove duplicate masks between crops
        if len(crop_boxes) > 1:
            # Prefer masks from smaller crops
            scores = 1 / box_area(data["crop_boxes"])  # 以box源自的crop之面积为分数，面积越小，分数越大。
            scores = scores.to(data["boxes"].device)
            keep_by_nms = batched_nms(
                data["boxes"].float(),
                scores,
                torch.zeros_like(data["boxes"][:, 0]),  # categories
                iou_threshold=self.crop_nms_thresh,  # 注意crop间nms阈值，与crop内nms阈值，可以单独设置。
            )
    ...
    def _process_crop(
        ...
        keep_by_nms = batched_nms(
            ...
            data["iou_preds"],  # BBox分数就是mask的iou预测。
            torch.zeros_like(data["boxes"][:, 0]),  # categories
    ...
    def _process_batch(
        ...
        if self.pred_iou_thresh > 0.0:
            keep_mask = data["iou_preds"] > self.pred_iou_thresh  # 预测iou要大于阈值。
    ...
    def postprocess_small_regions(
        ...
        new_masks = []
        scores = []
        for rle in mask_data["rles"]:  # 对每个mask填洞、削岛。
            mask = rle_to_mask(rle)

            mask, changed = remove_small_regions(mask, min_area, mode="holes")  # 填洞。
            unchanged = not changed
            mask, changed = remove_small_regions(mask, min_area, mode="islands")  # 削岛。
        ...
        masks = torch.cat(new_masks, dim=0)  # 重做NMS。
