...
4. Transformer：
  a. 总是无梯度前向。
  b. 收集、返回att weight。
  c. Vision最后一个layer不前向。
对应encode_text要接收att_weight——尽管用不上。
5. VisionTrans:
  a. 前向时，支持emb缩放。
  b. 最后一层不前向。
  c. 返回att_weight。
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        attn_weights = []
        with torch.no_grad():
            layers = self.layers if x.shape[0] == 77 else self.layers-1  # 图像编码器最后一个block此时不前向。
...
class CLIP(nn.Module):
    ...
    def forward_last_layer(self, image_features, text_features):
        ...
        logits_per_image = logit_scale * image_features @ text_features.t()  # (N, C)
