...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        for image in ms_imgs:
            ...
            text_features_temp = torch.cat([fg_features_temp, bg_features_temp], dim=0)  # 只有标签前景+背景参与CAM计算。
            ...
            for idx, label in enumerate(label_list):
                ...
                grayscale_cam_highres = cv2.resize(grayscale_cam, (ori_width, ori_height))  # (ori_h, ori_w)
                highres_cam_to_save.append(torch.tensor(grayscale_cam_highres))  # 记录上采样到原尺寸的CAM。

                if idx == 0:  # 同一张图片，第一次推理时计算affinity。
                    attn_weight_list.append(attn_weight_last)
                    attn_weight = [aw[:, 1:, 1:] for aw in attn_weight_list]  # (网络layer数, hw, hw)
                    attn_weight = torch.stack(attn_weight, dim=0)[-8:]
                    attn_weight = torch.mean(attn_weight, dim=0)  # 平均最后8个layer的att weight。
                    attn_weight = attn_weight[0].cpu().detach()  # (L, L)
                ...
                for i_ in range(cnt):  # BBox内区域，才被聚合。
                ...
                for _ in range(2):
                    ...
                    trans_mat = trans_mat / torch.sum(trans_mat, dim=1, keepdim=True)
                trans_mat = (trans_mat + trans_mat.transpose(1, 0)) / 2  # sink-horn + 对称平均，得到affinity。

                for _ in range(1):
                    trans_mat = torch.matmul(trans_mat, trans_mat)  # 聚合两次。
                ...
                cam_refined = cam_refined.cpu().numpy().astype(np.float32)  # 用affinity聚合优化。
                cam_refined_highres = scale_cam_image([cam_refined], (ori_width, ori_height))[0]  # 重新归一化并上采样。
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cam_out_dir', type=str,
                        default=f'experiment/others/CLIP-ES/baseline/{get_local_time_str(True)}')
