...
def crf(n_jobs, is_coco=False):
    ...
    def process(i):
        ...
        bg_score = np.power(1 - np.max(cams, axis=0, keepdims=True), args.power)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dt', default='voc', type=str, choices=['voc', 'coco'])
    parser.add_argument("--cam_out_dir", default="./cam_out", type=str)
    parser.add_argument("--cam_type", default="attn_highres", type=str)
    parser.add_argument("--power", default=1, type=int)
    parser.add_argument("--pseudo_mask_save_path", default="experiment/CLIP-ES/crf-baseline", type=str)
    parser.add_argument("--split_file", default="others/CLIP-ES/voc12/train_aug.txt",
                        type=str)
    parser.add_argument("--cam_eval_thres", default=2, type=float)
    parser.add_argument("--gt_root", default="datasets/VOC2012/SegmentationClassAug", type=str)
    parser.add_argument("--image_root", default="datasets/VOC2012/JPEGImages", type=str)
    parser.add_argument("--eval_only", action="store_true")
    parser.add_argument("--n_jobs", default=12, type=int)
    args = parser.parse_args()

    args.pseudo_mask_save_path = osp.join(args.pseudo_mask_save_path, osp.basename(args.cam_out_dir))

    is_coco = args.dt == 'coco'
    if args.dt == 'voc':
